...
- **Bezpečnosť sťahovania:** (1) všetko cez HTTPS na GitHub hosty (`IsTrustedDownloadHost` — `github.com`,
  `*.githubusercontent.com`); cudzí/nešifrovaný host sa odmietne. (2) Po stiahnutí sa overí **SHA-256**
  voči `digest` z GitHub API — nezhoda zruší inštaláciu (`UpdVerifyFailed`). *Pozor na rámec dôvery:* hash
  z GitHub API má rovnaký koreň dôvery ako asset, takže rieši integritu (poškodený download, CDN-tamper),
  **nie** kompromitáciu GitHub účtu. Verejné repo neznamená, že hocikto môže vydať release — len vlastník/
  kolaboranti s write právami. Skutočnú autenticitu (aj proti kompromitácii účtu) by dal až **podpis exe
  (Authenticode)** + overenie pred spustením — zatiaľ neimplementované (vyžaduje certifikát).
